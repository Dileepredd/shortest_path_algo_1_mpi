...
int main(int argc,char** argv)
{
    ...
    */
    ...
    int (*a)[nodes/procs][nodes];
    a = (int (*)[nodes/procs][nodes])new int[(nodes/procs)*nodes];
    fseek(fp,rank*(nodes/procs)*nodes*sizeof(int),SEEK_CUR);
    fread(a,(nodes/procs)*nodes,sizeof(int),fp);
    // fclose(fp);
    // matrix b,c,d declaration and initialization.
    int (*b)[nodes/procs][nodes];
    int (*c)[nodes/procs][nodes];
    b = (int (*)[nodes/procs][nodes])new int[(nodes/procs)*nodes];
    c = (int (*)[nodes/procs][nodes])new int[(nodes/procs)*nodes];
    ...
    */
    ...
    int temprank = rank;
    starttime = MPI_Wtime();
    if(source>=start && source<end && (*a)[source][destination] > 0)
    {
        ...
        for(int j=0;j<procs;j++)
        {
            for(int r=0;r<(nodes/procs);r++)
            {
                for(int k=0;k<nodes;k++)
                {
                    ...
                    {
                        (*c)[r][k] += (*b)[r][j*(nodes/procs)+l]*(*a)[l][k];
                    }
                }
            }
            // update the a
            if(feof(fp) == true)
            {
                fseek(fp,1*sizeof(int),SEEK_SET);// initial 4 bites are used for storing number of nodes.
            }
            fseek(fp,temprank*(nodes/procs)*nodes*sizeof(int),SEEK_CUR);
            fread(a,(nodes/procs)*nodes,sizeof(int),fp);
        }
        if(source>=start && source<end)
        {
            if((*c)[source-start][destination] > 0)
            {
